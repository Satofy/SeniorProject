                    ...
                    {team.members && team.members.length > 0 ? (
                      <div className="space-y-2">
                        {team.members.map((member) => (
                          <div
                            key={typeof member === 'string' ? member : member.id}
                            className="flex items-center justify-between p-3 border border-border rounded-lg"
                          >
                            <div>
                              <p className="font-medium">{typeof member === 'string' ? member : (member.email || member.id)}</p>
                              <p className="text-sm text-muted-foreground capitalize">{typeof member === 'string' ? 'player' : (member.role || 'player')}</p>
                            </div>
                            {(typeof member !== 'string' ? member.id : member) !== team.managerId && (
