...
export default function TournamentDetailPage() {
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      try {
        ...
        const data = await api.getTournament(String(id))
        setTournament(data)
      } catch (error) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await api.registerForTournament(String(id), isTeamManager ? selectedTeamId : undefined)
                ...
                {tournament.game && (
                  <div className="flex items-center gap-3 p-4 bg-muted/50 rounded-lg">
                    <div>
                      ...
                      <div className="pt-1"><GameBadge game={tournament.game} /></div>
                    </div>
                  </div>
                )}
